...
class SerialPortSettingsAdapter(private val binding : SerialPortSettingsDialogBinding) {
    fun initSettings() {
        ...
        // 读取本地串口值赋值给输入框
        val localCallSerialPort = sharedPreferencesManager.readString(
            Constants.SerialPort.call,
            Constants.SerialPort.Default.call
        )
        binding.callSerialPort.setText(localCallSerialPort)

        val localAirConditionSerialPort = sharedPreferencesManager.readString(
        ...
        val localControlSerialPort = sharedPreferencesManager.readString(
            Constants.SerialPort.control,
            Constants.SerialPort.Default.control
        )
        binding.controlSerialPort.setText(localControlSerialPort)

        val localMusicSerialPort = sharedPreferencesManager.readString(
