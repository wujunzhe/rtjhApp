...
class BottomAdapter(private val binding:BottomBinding):OnDataReceivedListener {
    ...
    private var sharedPreferencesManager : SharedPreferencesManager = SharedPreferencesManager(binding.root.context)
    ...
    private fun setTimeSettings(){
        if (controlSerialHelper.isOpen){
             // 消毒延时
            val disinfectTime = sharedPreferencesManager.readString(Constants.SmartSettings.disinfectTime,Constants.SmartSettings.Default.disinfectTime)
            ...
            // 照明延时
            val lightDelayTime = sharedPreferencesManager.readString(Constants.SmartSettings.lightDelayTime,Constants.SmartSettings.Default.lightDelayTime)
                        ...
                        lighting1Flag = if (! lighting1Flag) {
                            ...
                            if (lightDelayTime != null) {
                                if (lightDelayTime.toInt() != 0 && !lighting2Flag){
                                    MyToast().info(binding.root.context,"照明还有${lightDelayTime}分钟关闭")
                                }
                            }
                            false
                        }
