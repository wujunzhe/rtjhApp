...
class ModbusRtuMaster(private val serialHelper : SerialHelper) {
    ...
    private fun execute(
        ...
        when (functionCode) {
            ...
            else -> throw ModbusError(
                ModbusErrorType.ModbusFunctionNotSupportedError,
                "Not support function $functionCode"
            )
        }
    ...
    fun readCoils(slave : Int, startingAddress : Int, numberOfPoints : Int) {
        val sendBytes =
            execute(slave, ModbusFunction.READ_COILS, startingAddress, numberOfPoints, 0, null)
    ...
    fun writeSingleCoil(slave : Int, address : Int, value : Boolean) {
        val sendBytes =
            execute(slave, ModbusFunction.WRITE_SINGLE_COIL, address, 1, if (value) 1 else 0, null)
        serialHelper.send(sendBytes)
    }
    ...
    fun writeSingleRegister(slave : Int, address : Int, value : Int) {
        val sendBytes =
            execute(slave, ModbusFunction.WRITE_SINGLE_REGISTER, address, 1, value, null)
        serialHelper.send(sendBytes)
    }
    ...
    fun writeHoldingRegisters(slave : Int, address : Int, sCount : Int, data : IntArray) {
        val sendBytes =
            execute(slave, ModbusFunction.WRITE_HOLDING_REGISTERS, address, sCount, 0, data)
        serialHelper.send(sendBytes)
    }
